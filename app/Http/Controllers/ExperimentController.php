...
use App\Services\Esp32ExperimentTemplateService;
...
class ExperimentController extends Controller
{
    public function __construct(
        ...
        private Esp32ExperimentTemplateService $esp32Templates,
    ) {}
    ...
    public function createEsp32Drafts()
    {
        $this->authorizeAdmin();

        $result = $this->esp32Templates->createMissingDrafts(auth()->id());
        foreach ($result['created'] as $experiment) {
            $this->audit->log('experiment.esp32_draft_created', $experiment, [
                'tool_profile' => $experiment->tool_profile,
                'target_platform' => $experiment->target_platform,
            ]);
        }

        return redirect()->route('experiments.index', ['target_platform' => 'esp32'])
            ->with(
                'success',
                count($result['created']).' draft eksperimen ESP32 dibuat, '
    ...
    public function update(Request $request, Experiment $experiment)
    {
        ...
        $data['attack_pattern'] = $data['attack_pattern'] ?: ($profile['default_attack_pattern'] ?? null);
        $data['analysis_profile_key'] = $data['analysis_profile_key'] ?: $data['tool_profile'];
        // Provenance guard: editing metadata must never relabel historical VM evidence.
        $data['target_platform'] = $experiment->target_platform ?: 'esp32';
